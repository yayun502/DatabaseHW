<?php
    ...
    try{
        if(!isset($_POST['uname'])||!isset($_POST['uphone'])||!isset($_POST['uacc'])||!isset($_POST['pwd'])||!isset($_POST['ulat'])||!isset($_POST['ulon'])){
            header("Location: index.php");
            exit();
        }
        if(empty($_POST['uname'])||empty($_POST['uphone'])||empty($_POST['uacc'])||empty($_POST['pwd'])||empty($_POST['ulat'])||empty($_POST['ulon'])){
        ...
        if($stmt->rowCount()==0){
            ...
            $stmt = $conn->prepare("insert into users(name, phonenumber, account, password, salt, latitude, longtiude)
